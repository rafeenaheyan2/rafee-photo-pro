...
import { LOADING_IMAGE_URL, CLOTHING_OPTIONS } from './constants';
      ...
      <div className="min-h-screen bg-[#020202] text-white p-4 lg:p-10 flex flex-col animate-fade">
         ...
         <nav className="flex flex-wrap justify-between items-center mb-10 glass-panel p-6 rounded-[35px] gap-4">
            ...
            <div className="flex gap-3 flex-wrap items-center">
              ...
              <GlossyButton onClick={handleReselectClick} variant="secondary" className="px-6 py-3 lowercase tracking-normal font-medium normal-case">Reselect Photo</GlossyButton>
              {state.original && (
                <GlossyButton onClick={() => {
                  const link = document.createElement('a');
                  link.href = state.edited || ''; link.download = `rafee-pro-${Date.now()}.png`; link.click();
                }} className="px-10 py-3">Download HD</GlossyButton>
              )}
            </div>
         </nav>
               ...
               <div className="glass-panel p-8 rounded-[45px] space-y-8 border-white/5 shadow-2xl">
                  <div>
                    ...
                    <textarea 
                      ...
                      placeholder="Type neural command... (e.g., 'Change background to dark rainy street')"
            ...
            <div className="glass-panel p-10 rounded-[55px] space-y-10 shadow-2xl animate-fade">
                <div>
                  ...
                  <div className="grid grid-cols-1 gap-4">
                    <GlossyButton onClick={() => handleAction("Remove background perfectly and replace with clean white")} isLoading={state.isProcessing} variant="secondary" className="w-full py-5">Clean Background</GlossyButton>
                    <GlossyButton onClick={() => handleAction("Professional Passport photo, light blue background, formal attire")} isLoading={state.isProcessing} className="w-full py-5">Auto Passport</GlossyButton>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-3 gap-3 max-h-[280px] overflow-y-auto pr-2 custom-scrollbar">
                    {CLOTHING_OPTIONS.filter(opt => opt.gender === activeGenderTab).map((opt) => (
              ...
              {state.isProcessing ? (
                ...
                <div className="flex-1 flex flex-col p-12 md:p-16 animate-fade">
                  <div className="flex justify-between items-center mb-12">
                     ...
                     }} className="px-16 py-4 rounded-[20px]">Save Photo</GlossyButton>
                  </div>
                  <div className="flex-1 flex justify-center items-center">
